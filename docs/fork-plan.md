...
> **2026-08-18 方向修订（用户决策）**：离线 ASR/TTS 逐步**转云端语音 API**（用户自有语音
> API 资源），端侧 sherpa/Kokoro/Piper 链路退场——顺带绕开上游 Android 离线 ASR native
> 崩溃 P0；录音/音频将上传至所选 API（家庭自用可接受，替代上文「不上传音频」约束）。
> 词典保留本地方案。过渡期保险：CDN 全量备份已落地（F3，`D:\apps\backup\echo-loop-cdn\`）。
